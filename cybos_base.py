...
def buy_ioc_v2(company_code,buy_quantity, buy_price) :
    #### 초기화
    instCheck = cpTradeUtil.TradeInit(0)
    if (instCheck != 0):
        print("Initialization Fail")
    ...
    # # 매수 완료 종목 메세지 보내기
    # for stock in stocks:
    #     text = '종목명 : {} / 평균가격 : {} / 매수 완료 '.format(stock['name'], buy_price)
...
# CpMarketEye_v2_before : 장전 예상체결 리스트 추출
def CpMarketEye_v2_before(codes):
    # 연결 여부 체크
    ...
    31:예상체결수량(ulong)
    '''
    rqField = [0, 1, 2, 3, 4, 5, 8, 9, 10, 11, 22, 24, 13, 14, 15, 16]  # 요청 필드
